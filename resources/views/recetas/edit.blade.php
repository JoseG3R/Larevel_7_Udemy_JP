...
<div class="row justify-content-center mt-5">
    <div class="col-md-8">
        <form 
        action="{{route('recetas.update',['receta' => $receta->id])}}" method="POST" enctype="multipart/form-data" novalidate>
        @csrf
        @method('PUT')
            <div class="form-group">
                <label for="titulo">{{$receta->titulo}}</label>
                <input type="text" 
                    name="titulo" 
                    class="form-control @error('titulo') is-invalid @enderror" 
                    id="titulo" 
                    placeholder="Título receta"
                    value="{{ $receta->titulo, old('titulo') }}"
                    >
                @error('titulo')
            ...
            <div class="form-group mt-3">
                ...
                <input type="hidden" id="preparacion" name="preparacion" 
                    value="{{ $receta->preparacion,old('preparacion') }}">
                <trix-editor 
                    input="preparacion"
            ...
            <div class="form-group mt-3">
                ...
                <input type="hidden" id="ingredientes" name="ingredientes" 
                    value="{{ $receta->ingredientes, old('ingredientes') }}">
                <trix-editor 
                    input="ingredientes"
